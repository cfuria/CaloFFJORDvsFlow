...
def plot_layer_energy(tensor, fp, layer, save_it=True, plot_ref=None, epoch_nr=None,
                      lower_threshold=0., ref_thres=None, plot_GAN=None, e_ratio=False):
    ...
    #bins = np.linspace(0, 1e3, 50) #default binning
    ...
    if plot_ref is not None:
        ...
        if plot_ref in ['eplus', 'gamma', 'piplus']:
            _, _, ref_e_layer = preprocessing.DataLoaderDataset2(file_path, 1000, inc_noise=False)
            #print("Reference E Layer:")
            #print(ref_e_layer[0]) 
            #ref_energy, ref_energy_voxel, ref_e_layer = preprocessing.DataLoaderDataset2(file_path, 1000)
            #true_energy, ref_energy_voxel = preprocessing.ReverseNormChallenge(ref_energy, ref_energy_voxel, ref_e_layer)
            #ref_energy_voxel = ref_energy_voxel.reshape(ref_energy_voxel.shape[0],9,-1)
            reference = np.reshape(ref_e_layer[:, layer],(ref_e_layer[:, layer].shape[0], -1))
            ...
            if plot_ref == 'eplus':
                ...
                label = r'$\pi^+$ GEANT'
                leg_loc = 'upper right'
            bins = np.geomspace(np.quantile(reference,0.01) + 1e-8,np.quantile(reference,1.0),10)
...
def plot_total_energy(tensor, fp, save_it=True, plot_ref=None, epoch_nr=None,
                      lower_threshold=1e-7, ref_thres=None, plot_GAN=None):
    ...
    data = tensor.clamp_(0., 1e5).to('cpu').numpy()
    data = np.where(data < lower_threshold, np.zeros_like(data), data)
    assert data.shape[-1] == 9 * 8 * 8, (
        "Are you sure the input of shape {} is from cells of all 8 layers?".format(tensor.size()))
    energies = data.sum(axis=-1)
    energies = energies #* 1000.0
    #bins = np.linspace(0, 1e3, 10) #(0,1e3,50)
    bins = np.geomspace(1, 1e3, 8)
    file_path = '/global/cfs/cdirs/m3929/SCRATCH/FCC/dataset_2_2_small.hdf5'
    colors = matplotlib.cm.gnuplot2(np.linspace(0.2, 0.8, 3))
    plt.figure(figsize=(6, 6))
    ...
    if plot_ref is not None:
        ...
        if ref_thres not in [0., 1e-1, 1e-2, 1e-3, 1e-4]:
            raise ValueError(
                "lower_threshold {} not in pre-computed set for plot_ref".format(ref_thres))
        if plot_ref in ['eplus', 'gamma', 'piplus']:
            ref_energy, _, _= preprocessing.DataLoaderDataset2(file_path, 10000, inc_noise=False)
            print("Total energy")
            print(ref_energy[0])
            #convert it back
            #ref_energy_voxel[ref_energy_voxel<1e-4]=0
            reference = np.reshape(ref_energy,(ref_energy.shape[0], -1))
            ...
            if plot_ref == 'eplus':
                ...
                color = colors[2]
                label = r'$\pi^+$ GEANT'
            _ = plt.hist(reference, bins=bins, histtype='stepfilled', #bins=bins,
                         linewidth=2, alpha=0.2, density=True, color=color,
                         label=label)
        else:
            raise ValueError("'{}' is not a valid reference name".format(plot_ref))
    ...
    _ = plt.hist(energies, bins=bins, histtype='step', linewidth=3, #bins=bins,
                 alpha=1, color=color if plot_ref is not None else colors[0], density='True',
                 label='CaloFlow')
    plt.xscale('log')
    if epoch_nr is not None:
...
def plot_layer_E_ratio(tensor, fp, layer, save_it=True, plot_ref=None, epoch_nr=None,
    ...
    data = tensor.clamp_(0., 1e5).to('cpu').numpy()
    data = data.reshape(data.shape[0], -1)
    data = np.where(data < lower_threshold, np.zeros_like(data), data) #lower_threshold
    top2 = np.sort(data, axis=1)[:, -2:]
    E_ratio = ((top2[:, 1] - top2[:, 0]) / (top2[:, 0] + top2[:, 1]))[top2[:, 1] > 0]
    ...
    if plot_ref is not None:
        ...
        if plot_ref in ['eplus', 'gamma', 'piplus']:
            ...
            ref_energy, ref_voxel, ref_e_layer = preprocessing.DataLoaderDataset2(file_path, 1000, inc_noise=False)
            _, ref_voxel = preprocessing.ReverseNormChallenge(ref_energy, ref_voxel, ref_e_layer)
            ref_voxel = 1e3*ref_voxel[:,layer*64:(layer + 1)*64].reshape(ref_voxel.shape[0],-1)
            ref_voxel[ref_voxel<lower_threshold]=0
            ref_top2 = np.sort(ref_voxel, axis=1)[:, -2:]
            reference = ((ref_top2[:, 1] - ref_top2[:, 0]) / (ref_top2[:, 0] + ref_top2[:, 1]))[ref_top2[:, 1] > 0]
